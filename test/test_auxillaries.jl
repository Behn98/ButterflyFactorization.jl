@testitem "Testing custom Blockconstructions" begin
    ...
    M = ButterflyFactorization.blockdiag(M1, M2)
    @test size(M) == (26, 22)

    M = SparseArrays.sparse(M)
    sparseM = ButterflyFactorization.sparse_blockdiag(M, M2)
    ...
    sparseM3 = ButterflyFactorization.sparse_vcat(M3, sparseM)
    ...
    M = ButterflyFactorization.blocksparse_blockdiag(M1, M2)
    @test size(M) == (26, 22)
    ...
    M = ButterflyFactorization.blocksparse_vcat(M1[:, 1:10], M2)
    @test size(M) == (26, 10)
    @test size(M.blocks)[1] == 2
...
@testitem "Testing Subdictionary functionality" begin
    ...
    for i in 1:10
        @test ButterflyFactorization.getsubdict!(R1, idx1[i]) == R1[idx1[i]]
        @test ButterflyFactorization.getsubdict!(R2, idx2[i]) == R2[idx2[i]]
    end
    ...
    @test rows == ButterflyFactorization.find_rows_for_column(R2, col_idx)
    rows = []
    col_idx = idx1[2]
    for row in keys(R1)
    ...
    @test rows == ButterflyFactorization.find_rows_for_column(R1, col_idx)
end
...
@testitem "Testing Subdictionary functionality" begin
    ...
    for i in eachindex(tree1.testcluster.nodesatlevel)
        @test issetequal(
            ButterflyFactorization.h2treelevels(
    ...
    paddedtlvls = ButterflyFactorization.traverseandpad(
        tree1.testcluster, H2Trees.root(tree1.testcluster)
    )
    ml = length(paddedtlvls)
    @test issetequal(paddedtlvls[ml], leaves(tree1.testcluster))
    nonpaddedtlvls = ButterflyFactorization.h2treelevels(
        tree1.testcluster, H2Trees.root(tree1.testcluster)
    )
    ml2 = length(nonpaddedtlvls)
    @test !issetequal(nonpaddedtlvls[ml], leaves(tree1.testcluster))
end
