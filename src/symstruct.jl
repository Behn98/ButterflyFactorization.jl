...
struct BF3
    ...
    k::Float64
    τ::Float64
    BF3(Q, R, P, tree, dim, level, NS, NO, k, τ) =
        new(Q, R, P, tree, dim, level, NS, NO, k, τ)
end
...
function Base.size(A::ButterflyFactorization.BF3, dim=nothing)
    if dim === nothing
        return (A.dim[1], A.dim[2])
    elseif dim == 1
        return A.dim[1]
...
function Base.length(A::ButterflyFactorization.BF3)
    return A.level + 2
end
